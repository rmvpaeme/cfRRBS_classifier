...
#data_path <- "/Users/rmvpaeme/Repos/cfRRBS_classifier_v0.5/classifySamples/output/classification/"
#outdir <- "./classifySamples/output/plots/"
#normal <- "wbc,normal"
...
grouped_plotbar <- grouped

p1 <- ggplot(grouped_plotbar, aes(x = V1)) + geom_bar() + facet_wrap(~variable, scales="free") + coord_flip()
...
tgc <- as.data.table(tgc)

p2 <- ggplot(tgc, aes(x = V1, y = value*100)) +
  geom_point() + theme_bw() + facet_wrap(~ variable, scales = "free") + geom_errorbar(aes(ymin=value*100-ci*100, ymax=value*100+ci*100), width=.1) +
  theme(panel.grid.major.y=element_line(linetype = "dashed",color="gray88"), axis.text.y=element_text(size=8)) + 
  labs(y = "Estimated TFx (%) (mean +/- 95%CI)", x = "SampleID", col = "Classification call") + coord_flip() 
ggsave(paste0(opt$outdir, "/", format(Sys.time(),'%Y%m%d_%H%M%S_'), "errorpersample.png"), p2, width = 16, height = 16, limitsize = FALSE)

tgc <- tgc[tgc[, .I[value == max(value)], by=variable]$V1]
classificationResults_plasma <- as.data.frame(tgc)
